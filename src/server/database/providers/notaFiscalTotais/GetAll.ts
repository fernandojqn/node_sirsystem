...
export const getAll = async (page: number, limit: number, filter: string, id = 0, empresaId: number): Promise<INotaFiscalTotais[] | Error> => {
    ...
    try {
        ...
        if (id > 0 && result.every(item => item.id !== id)) { 
            const resultById = await Knex(ETableNames.notaFiscalTotais)
                .select('*')
                .where('id', '=', id)
                .first();
