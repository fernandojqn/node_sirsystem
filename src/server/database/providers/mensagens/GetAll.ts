...
export const getAll = async (page: number, limit: number, filter: string, id = 0, empresaId: number): Promise<IMensagem[] | Error> => {

    try {
        const result = await Knex(ETableNames.mensagens)
            .select('*') // seleciona tudo
            .where('id', Number(id)) //que tenha esse id
            .orWhere('mensagemImpressao', 'like', `%${filter}%`) // "ou" que o nome "seja igual" ao filtro
