...
export const getAll = async (page: number, limit: number, filter: string, id = 0, empresaId: number): Promise<ITributacao[] | Error> => {

    try {
        const result = await Knex(ETableNames.tributacoes)
            .select('*') // seleciona tudo
            .where('id', Number(id)) //que tenha esse id
            .orWhere('regra', 'like', `%${filter}%`) // "ou" que o nome "seja igual" ao filtro
        ...
        if (id > 0 && result.every(item => item.id !== id)) { 
            const resultById = await Knex(ETableNames.tributacoes)
                .select('*')
                .where('id', '=', id)
                .first();
