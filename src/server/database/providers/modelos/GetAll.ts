...
export const getAll = async (page: number, limit: number, filter: string, id = 0): Promise<IModelo[] | Error> => {

    try {
        const result = await Knex(ETableNames.modelos)
            .select('*') // seleciona tudo
            .where('id', Number(id)) //que tenha esse id
            .orWhere('modeloDescricao', 'like', `%${filter}%`) // "ou" que o nome "seja igual" ao filtro
        ...
        if (id > 0 && result.every(item => item.id !== id)) { 
            const resultById = await Knex(ETableNames.modelos)
                .select('*')
                .where('id', '=', id)
                .first();
