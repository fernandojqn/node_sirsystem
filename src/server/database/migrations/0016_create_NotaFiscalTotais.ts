...
export async function up(knex: Knex) {
    return knex
        .schema
        .createTable(ETableNames.notaFiscalTotais, table => {
            table.bigIncrements('id').primary().index();

            table.bigInteger('pedidoId').nullable().defaultTo(null).index().references('id')
                .inTable(ETableNames.notaFiscalTotais).onUpdate('CASCADE').onDelete('RESTRICT');
            ...
            table.string('descricaoAvista', 100).checkLength('<=', 100);
            ...
            table.string('numeroEmbalagem', 30).checkLength('<=', 30);
            table.float('quantidadeEmbalagem');
            table.string('especieEmbalagem', 30).checkLength('<=', 30);
            table.float('pesoLiquidoEmbalagem');
            table.float('pesoBrutoEmbalagem');
            table.string('marca', 30).checkLength('<=', 30);

            table.float('codigoMensagemId');
            ...
            table.comment('Tabela usada para armazenar Nota Fiscal Totais.');
        })
        .then(() => {
            console.log(`# Created table ${ETableNames.notaFiscalTotais}`);
        });
}
...
export async function down(knex: Knex) {
    return knex
        .schema
        .dropTable(ETableNames.notaFiscalTotais)
        .then(() => {
            console.log(`# Dropped table ${ETableNames.notaFiscalTotais}`);
        });
}